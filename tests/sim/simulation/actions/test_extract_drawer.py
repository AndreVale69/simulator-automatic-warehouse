...
class TestExtractDrawer(TestCase):
    def test_simulate_action_carousel_empty(self):
        # arrange
        warehouse = Warehouse()
        warehouse.cleanup_carousel()
        simulation = WarehouseSimulation(warehouse)
        action = ExtractDrawer(simulation.env, simulation.warehouse, simulation)

        # act
        simulation.env.process(action.simulate_action(destination=EnumWarehouse.CAROUSEL))
        simulation.env.run()
        ...
        self.assertEqual(ActionEnum.EXTRACT_DRAWER.value, action_performed['Type of Action'])
    ...
    def test_simulate_action_bay_occupied(self):
        ...
        self.assertIsInstance(simulation.warehouse.get_carousel().get_buffer_drawer(), Drawer)
